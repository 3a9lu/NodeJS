...
const db = 'Link on MongoDB';

mongoose
	.connect(db, {useNewUrlParser: true, useUnifiedTopology: true})
	.then((res) => console.log('Подключение к базе данных.'))
	.catch((error) => console.log(error));

const createPath = (page) => path.resolve(__dirname, 'ejs', `${page}.ejs`);

app.listen(PORT, (error) => {
	error ? console.log(error) : console.log(`Сервер начал прослушивание запросов на порту ${PORT}.`);
});
